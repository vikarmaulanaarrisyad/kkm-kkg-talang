...
Pastikan output HANYA JSON murni tanpa ada teks lain di luarnya.`;
    ...
    return NextResponse.json({ error: (error.message || '').includes('429') || (error.message || '').includes('Quota') || (error.message || '').includes('503') || (error.message || '').includes('high demand') ? 'Batas penggunaan AI gratis harian telah habis atau server Google sedang sibuk. Mohon coba kembali besok atau beberapa saat lagi.' : (error.message || 'Terjadi kesalahan saat men-generate ATP') }, { status: 500 });
  }
}
