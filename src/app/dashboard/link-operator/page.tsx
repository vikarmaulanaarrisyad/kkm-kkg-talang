...
import { Plus, Trash2, Edit, Loader2, Link as LinkIcon, AppWindow, Database, Globe, Smartphone, Monitor } from "lucide-react";
import Swal from "sweetalert2";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

export default function LinkOperatorPage() {
  const [items, setItems] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  // Form State
  const [formData, setFormData] = useState({ id: "", title: "", url: "", icon_type: "Globe" });
  ...
  const fetchItems = async () => {
    try {
      const res = await fetch("/api/link-operator");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = isEditing ? `/api/link-operator/${formData.id}` : "/api/link-operator";
  ...
  const handleDelete = async (id: string, title: string) => {
    const result = await Swal.fire({
      title: 'Hapus Link?',
      text: `Anda yakin ingin menghapus link "${title}"?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        const res = await fetch(`/api/link-operator/${id}`, { method: "DELETE" });
        const json = await res.json();
  ...
  const handleEdit = (item: any) => {
    setFormData({
      ...
      title: item.title,
      url: item.url,
      icon_type: item.icon_type || "Globe"
    });
    setIsEditing(true);
  };

  const resetForm = () => {
    setFormData({ id: "", title: "", url: "", icon_type: "Globe" });
    setIsEditing(false);
  };
  ...
  const getIconPreview = (type: string) => {
    switch(type) {
      ...
      case 'AppWindow': return <AppWindow className="w-5 h-5 text-emerald-600" />;
      ...
      case 'Monitor': return <Monitor className="w-5 h-5 text-slate-600" />;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Kelola Link Aplikasi</h1>
          <p className="text-sm text-slate-500">Kelola kumpulan link atau aplikasi yang sering digunakan oleh operator.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h2 className="text-lg font-bold text-slate-800 mb-4">{isEditing ? "Edit Link" : "Tambah Link Baru"}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Nama Aplikasi / Link</label>
                <Input 
                  placeholder="Misal: Portal EMIS, Simpatika, RDM" 
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">URL Tujuan</label>
                <Input 
                  placeholder="https://..." 
                  value={formData.url} 
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="Globe">Website / Globe (Biru)</option>
                  <option value="AppWindow">Aplikasi (Hijau)</option>
                  <option value="Database">Database (Ungu)</option>
                  <option value="Smartphone">Mobile App (Oranye)</option>
                  <option value="Monitor">Sistem / Monitor (Abu-abu)</option>
                </select>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-slate-500 uppercase bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    <th className="px-6 py-4 font-semibold">Nama Aplikasi</th>
                    ...
                    <tr>
                      <td colSpan={3} className="px-6 py-12 text-center text-slate-500">
                        <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          <p>Belum ada data link aplikasi.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
